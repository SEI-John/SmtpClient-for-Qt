...
QString MimeMessage::toString()
{
    ...
    if (sender->getName() != "")
    {
        ...
        case MimePart::Base64:
            mime += " =?utf-8?B?" + QByteArray().append(sender->getName().toStdString()).toBase64() + "?=";
            break;

        case MimePart::QuotedPrintable:
            mime += " =?utf-8?Q?" + QuotedPrintable::encode(QByteArray().append(sender->getName().toStdString())).replace(' ', "_").replace(':', "=3A") + "?=";
            break;
    ...
    for (i = 0, it = recipientsTo.begin(); it != recipientsTo.end(); ++it, ++i)
    {
        ...
        if ((*it)->getName() != "")
        {
            ...
            {
            case MimePart::Base64:
                mime += " =?utf-8?B?" + QByteArray().append((*it)->getName().toStdString()).toBase64() + "?=";
                break;

            case MimePart::QuotedPrintable:
                mime += " =?utf-8?Q?" + QuotedPrintable::encode(QByteArray().append((*it)->getName().toStdString())).replace(' ', "_").replace(':', "=3A") + "?=";
                break;
    ...
    {
    case MimePart::Base64:
        mime += "=?utf-8?B?" + QByteArray().append(subject.toStdString()).toBase64() + "?=";
        break;

    case MimePart::QuotedPrintable:
        mime += "=?utf-8?Q?" + QuotedPrintable::encode(QByteArray().append(subject.toStdString())).replace(' ', "_").replace(':', "=3A") + "?=";
        break;
    ...
    if (replyTo)
    {
        ...
        if (replyTo->getName() != "")
        {
            ...
            {
            case MimePart::Base64:
                mime += " =?utf-8?B?" + QByteArray().append(replyTo->getName().toStdString()).toBase64() + "?=";
                break;

            case MimePart::QuotedPrintable:
                mime += " =?utf-8?Q?" + QuotedPrintable::encode(QByteArray().append(replyTo->getName().toStdString())).replace(' ', "_").replace(':', "=3A") + "?=";
                break;
